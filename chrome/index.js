...
window.onload = () => {
  if (chrome != undefined) {
    chrome.storage.local.get("temp").then((results) => {
      if (results.temp == undefined) {
        chrome.storage.local.get("Notes").then((results) => {
...
function composeCollectionMenu(options = config) {
  ...
  if (!document.getElementById("collection-menu")) {
    ...
    chrome.storage.local.get("Archive").then((results) => {
      if (
        Object.keys(results.Archive) != undefined &&
        Object.keys(results.Archive).length > 0
      ) {
    ...
    text_input.addEventListener(
      ...
      function (e) {
        e.preventDefault();
        let input = document
          .getElementsByClassName("collection-text-input")[0]
          .value.trim();
        let list = Array.from(
          document.getElementsByClassName("collection-menu-button")
        );
        let xx;
        ...
        for (i = 0; i < list.length; i++) {
          ...
          tar.SetHeightWidth = function (target) {
            ...
            tar.style.left = tar.style.clientY;
          };
          ...
          if (text.indexOf(input) < 0) {
            ...
            if (xx == 0) {
              xx = 1;
              tar.style.transform = `translate(-100px, ${getRandomArbitrary(
                -100,
                100
              )}px)`;
            } else {
      ...
      Swal.fire({
        ...
        if (results.isConfirmed) {
          ...
          if (hold.Archive != undefined) {
            ...
            res[name] = copy;
            chrome.storage.local.set({ Archive: res }).then(showArchive());
          } else {
            chrome.storage.local.set({ Archive: final }).then(showArchive());
          }
        } else {
          accum--;
        }
