...
window.onload = () => {
  if (browser != undefined) {
    browser.storage.local.get("temp").then((results) => {
      if (results.temp == undefined) {
        browser.storage.local.get("Notes").then((results) => {
...
function composeCollectionMenu(collection = null, options = config) {
  ...
  if (!document.getElementById("collection-menu")) {
    ...
    browser.storage.local.get("Archive").then((results) => {
      ...
      Swal.fire({
        ...
        if (results.isConfirmed) {
          ...
          if (hold.Archive != undefined) {
            ...
            res[name] = copy;
            browser.storage.local.set({ Archive: res }).then(showArchive());
          } else {
            browser.storage.local.set({ Archive: final }).then(showArchive());
          }
        } else {
          accum--;
        }
